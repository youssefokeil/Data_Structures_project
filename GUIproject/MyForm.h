...
#pragma endregion
	...
	private: System::Void button4_Click(System::Object^ sender, System::EventArgs^ e) {//toJSON
		panel4->Show();
		FileStream^ selectedFileStream = gcnew FileStream(selectedFilePath, FileMode::Open, FileAccess::Read);
		StreamReader^ selectedFileReader = gcnew StreamReader(selectedFileStream);
		JSONConverter::ConvertToJSON(selectedFileReader, outputFilePath);
		DisplayOutputInTextBox(outputFilePath, readOnlyTextBox4);
	}
	...
	private: System::Void button13_Click_1(System::Object^ sender, System::EventArgs^ e) {//save as button panel4
		...
		saveFileDialog->DefaultExt = "json"; // Set default extension to .json
		saveFileDialog->AddExtension = true;
	...
	private: System::Void button6_Click(System::Object^ sender, System::EventArgs^ e) {//minify button	
		...
		FileStream^ selectedFileStream = gcnew FileStream(selectedFilePath, FileMode::Open, FileAccess::Read);
		StreamReader^ selectedFileReader = gcnew StreamReader(selectedFileStream);
		MinifyLib::Minifier::Minify(selectedFileReader,outputFilePath);
		DisplayOutputInTextBox(outputFilePath, readOnlyTextBox6);
	}
	...
	// Show the dialog and check if the user selected a file
		if (saveFileDialog->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
			...
			try {
				// Assume we have a method to get the current content as a string.
				String^ content = GetCurrentContent(readOnlyTextBox6); // Replace with your actual method to get content.
